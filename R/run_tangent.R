...
#'
#' Isolate tumor signal by removing systematic noise using a set of normal samples.
#'
#' @param sif_df Tibble or filepath to a text file containing sample metadata
#' @param nsig_df Tibble or filepath to a text file containing the normal signal matrix
#' @param tsig_df Tibble or filepath to a text file containing the tumor signal matrix
#' @param n_latent Number of latent factors to reconstruct normal subspace
#' @param make_plots If `TRUE`, generate plots of latent factor importance and effects of linear transformation
...
#'
#' @export
run_tangent <- function(sif_df, nsig_df, tsig_df, n_latent, make_plots = TRUE, output_dir = NULL) {
  # Load data
  ...
  if (make_plots) {
    plot_x_and_y_signal_transformation(
      sif, n.df, t.df,
      linear.transformation$n.df,
      linear.transformation$t.df,
      output_dir
    )
  }
  ...
  if (make_plots) plot_latent_factors(n.autox.svd, output_dir)
  ...
  if (num.lf == 0 | num.lf > length(n.autox.svd$d)) {
    stop('Number of latent factors needs to be greater than 0, and less than or equal to the number of normal samples.')
  } else if (num.lf == 1) {
  ...
  male.normal.samples <- sif %>%
    dplyr::filter(.data$sample.id %in% colnames(n.df) & .data$sex=='male') %>%
    dplyr::pull(.data$sample.id)
