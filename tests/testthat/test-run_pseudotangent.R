test_that("Running Pseudotangent with filepath inputs works", {
  ...
  pseudotangent_expect_out <- readRDS(testthat::test_path("dummydata", "PseudoTangent_test_output.rds"))
  pseudotangent_out <- run_pseudotangent(sif_path, normal_path, tumor_path, 5, num_partition = 2, n_latent_part = 2)
...
test_that("Running Pseudotangent with dataframe inputs works", {
  ...
  pseudotangent_expect_out <- readRDS(testthat::test_path("dummydata", "PseudoTangent_test_output.rds"))
  pseudotangent_out <- run_pseudotangent(sif, n.df, t.df, 5, num_partition = 2, n_latent_part = 2)
